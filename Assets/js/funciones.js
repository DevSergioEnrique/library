...
function btnEditarAdmin(id) {
    document.getElementById("title").textContent = "Actualizar Administrador";
    document.getElementById("btnAccion").textContent = "Modificar";
    const url = base_url + "Usuarios/editar/"+id;
    const http = new XMLHttpRequest();
    http.open("GET", url, true);
...
function cambiarPass(e) {
    ...
    if (clave.value == "" || confirmar.value == ""){
        alertas('Los campos son requeridos', 'warning');
    } else {
...
function registrarEstudiante(e) {
    ...
    const nombre = document.getElementById("nombre");
    const a_paterno = document.getElementById("a_paterno");
    const a_materno = document.getElementById("a_materno");
    const grado = document.getElementById("grado");
    const seccion = document.getElementById("seccion");
    if (codigo.value == "" || dni.value == "" || nombre.value == ""
    || a_paterno.value == "" || a_materno.value == "" || grado.value === "" 
    || seccion.value === "") {
        alertas('Todo los campos son requeridos', 'warning');
    } else {
...
function btnEditarEst(id) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById("nombre").value = res.nombre;
            document.getElementById("a_paterno").value = res.a_paterno;
            document.getElementById("a_materno").value = res.a_materno;
...
function btnBorrarEst(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const url = base_url + "Estudiantes/borrar/" + id;
            const http = new XMLHttpRequest();
            http.open("GET", url, true);
            http.send();
            http.onreadystatechange = function () {
...
function registrarMateria(e) {
    ...
    if (materia.value == "") {
        alertas('La categoría es requerida', 'warning');
    } else {
...
function btnEditPrestamo(id) {
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            const res = JSON.parse(this.responseText);
            document.getElementById("id").value = res.id;
            libro.value = res.libro;
            estudiante.value = res.estudiante;
            cantidad.value = res.cantidad;
            fecha_prestamo.value = res.fecha_prestamo;
            fecha_devolucion.value = res.fecha_devolucion;
            observacion.value = res.observacion;
            frm.setAttribute("onsubmit", "editPrestamo(event)")
            $("#prestar").modal("show");
        }
    }
}

function editPrestamo(e) {
    e.preventDefault();
    const clave = document.getElementById("clave");
    const confirmar = document.getElementById("confirmar");
